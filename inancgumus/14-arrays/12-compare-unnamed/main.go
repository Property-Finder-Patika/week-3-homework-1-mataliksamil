...
func main() {
	type (
		...
		//			   in this condition "integer" variable red cannot be converted to the bookcase which has int type instances
	)
	...
	if cabinet(blue) == red { // type conversation from bookcase to cabinet
		fmt.Println("✅")
	} else {
		fmt.Println("❌")
	}
